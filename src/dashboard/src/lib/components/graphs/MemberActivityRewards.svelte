<script lang="ts">
  import { graphMemberActivityRewards } from "$lib/api/monitor.api"
  ...
  onMount(async () => {
    graph = await graphMemberActivityRewards()
...
{#if ready}
  ...
  <table>
    <thead>
      <tr>
        <th>Milestone</th>
        <th>Members active 30 day avg.</th>
        <th>Points</th>
      </tr>
    </thead>
    <tbody>
